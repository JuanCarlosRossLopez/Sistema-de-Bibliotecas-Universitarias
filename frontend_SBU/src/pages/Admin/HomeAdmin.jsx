...
function HomeAdmin() {
    ...
    const fetchUserCounts = async () => {
        try {
            ...
            const admins = users.filter(user => user.Rol.name_rol === "admin").length;
            const employees = users.filter(user => user.Rol.name_rol === "employee").length;
                    ...
                    <div className="flex flex-col md:flex-row gap-4 justify-between w-full">
                        <div className="bg-[#C19B93] rounded-md flex-1 p-4 md:p-6 lg:p-8">
                            <h1 className="font-bold text-base md:text-lg lg:text-xl text-center md:text-left">
                                Administradores registrados: <b>{numAdmins}</b>
                            </h1>
                        </div>
                        <div className="bg-[#C19B93] rounded-md flex-1 p-4 md:p-6 lg:p-8">
                            <h1 className="font-bold text-base md:text-lg lg:text-xl text-center md:text-left">
                                Empleados registrados: <b>{numEmployees}</b>
                            </h1>
                        </div>
                    </div>
